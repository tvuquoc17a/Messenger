...
public class SignalRListener {
    ...
    public SignalRListener(String accessToken) {
        SignalRListener.accessToken = accessToken;
        ...
        hubConnection.on("ReceiveMessage", (message) -> {
            Log.d("ReceiveMessage", "message" + message);
        }, MessageDot.class);
    }
    ...
    public void sendMessage(MessageDot message, Integer roomId ){
        if(startConnection()){
            hubConnection.send("sendMessage", message, roomId);
        }
    }
}
